...
module ConstantContactClient
  ...
  class GetContactById200ResponseEmailAddress
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConstantContactClient::GetContactById200ResponseEmailAddress` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConstantContactClient::GetContactById200ResponseEmailAddress`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'opt_in_source')
        self.opt_in_source = attributes[:'opt_in_source']
      end
    ...
    def hash
      [address, permission_to_send, created_at, updated_at, opt_in_source, opt_in_date, opt_out_source, opt_out_date, opt_out_reason, confirm_status].hash
    end
