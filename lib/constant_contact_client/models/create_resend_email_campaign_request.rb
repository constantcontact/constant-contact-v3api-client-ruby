...
module ConstantContactClient
  class CreateResendEmailCampaignRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConstantContactClient::CreateResendEmailCampaignRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConstantContactClient::CreateResendEmailCampaignRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @resend_subject.nil?
        invalid_properties.push('invalid value for "resend_subject", resend_subject cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @resend_subject.nil?
      true
    end
    ...
    def hash
      [resend_subject, delay_days, delay_minutes].hash
    end
