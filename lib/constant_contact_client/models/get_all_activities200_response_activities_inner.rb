...
module ConstantContactClient
  ...
  class GetAllActivities200ResponseActivitiesInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConstantContactClient::GetAllActivities200ResponseActivitiesInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConstantContactClient::GetAllActivities200ResponseActivitiesInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'source_file_name')
        self.source_file_name = attributes[:'source_file_name']
      end
    ...
    def hash
      [activity_id, state, started_at, completed_at, created_at, updated_at, source_file_name, percent_done, activity_errors, status, _links].hash
    end
