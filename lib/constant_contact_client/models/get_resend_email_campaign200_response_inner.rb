...
module ConstantContactClient
  class GetResendEmailCampaign200ResponseInner
    # The subject line used when resending the email campaign activity.
    attr_accessor :resend_subject
    ...
    attr_accessor :resend_request_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConstantContactClient::GetResendEmailCampaign200ResponseInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConstantContactClient::GetResendEmailCampaign200ResponseInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'resend_subject')
        self.resend_subject = attributes[:'resend_subject']
      end

      if attributes.key?(:'delay_days')
        self.delay_days = attributes[:'delay_days']
      end
      ...
      if attributes.key?(:'resend_date')
        self.resend_date = attributes[:'resend_date']
      end

      if attributes.key?(:'resend_request_id')
        self.resend_request_id = attributes[:'resend_request_id']
      end

      if attributes.key?(:'resend_status')
        self.resend_status = attributes[:'resend_status']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          resend_subject == o.resend_subject &&
          delay_days == o.delay_days &&
          delay_minutes == o.delay_minutes &&
          resend_date == o.resend_date &&
          resend_request_id == o.resend_request_id &&
          resend_status == o.resend_status
    end
    ...
    def hash
      [resend_subject, delay_days, delay_minutes, resend_date, resend_request_id, resend_status].hash
    end
