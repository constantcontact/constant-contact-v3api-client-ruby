...
module ConstantContactClient
  class GetCampaignActivityLinkReport200ResponseLinkClickCountsInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConstantContactClient::GetCampaignActivityLinkReport200ResponseLinkClickCountsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConstantContactClient::GetCampaignActivityLinkReport200ResponseLinkClickCountsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'list_action')
        self.list_action = attributes[:'list_action']
      end
      ...
      if attributes.key?(:'link_tag')
        self.link_tag = attributes[:'link_tag']
      end
    end
    ...
    def hash
      [link_url, url_id, unique_clicks, list_action, list_id, link_tag].hash
    end
