  ...
  class UpdateContactRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConstantContactClient::UpdateContactRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConstantContactClient::UpdateContactRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @update_source.nil?
        invalid_properties.push('invalid value for "update_source", update_source cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @update_source.nil?
    ...
    def hash
      [email_address, first_name, last_name, job_title, company_name, birthday_month, birthday_day, anniversary, update_source, custom_fields, phone_numbers, street_addresses, list_memberships, taggings, notes].hash
    end
